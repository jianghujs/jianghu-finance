...
const commonUtil = require('@jianghujs/jianghu/app/common/commonUtil');
...
async function sqlResource({ jianghuKnex, ctx }) {
  ...
  const { ignoreTableListOfAppaId=[] } = ctx.app.config.jianghuConfig
  const appData = requestBody.appData || {};
  ...
  // 1. where 构建：前后端合并
  let whereCondition = await buildWhereCondition(jianghuKnex, ctx, requestBody);
  ...
  // 1.1. 添加 账套过滤appaId
  if (!ignoreTableListOfAppaId.includes(table)) {
    if (['select', 'delete', 'jhDelete', 'update', 'jhUpdate'].includes(operation) ) {
      whereCondition = `.where({ appaId: "${appaId}" })` + whereCondition;
    }
    if (['insert', 'jhInsert'].includes(operation) ) {
      actionData.appaId = appaId;
    }
  }
